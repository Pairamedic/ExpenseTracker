      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1 flex-wrap">
            <span className="text-sm text-slate-500">Balance: <span className="text-slate-300 font-medium">{formatCurrency(debt.balance)}</span></span>
            {debt.interestRate != null && (
              <span className="text-sm text-slate-500">{debt.interestRate}% APR</span>
            )}
          </div>
    ...
    <div className="pb-32">
      <div className="px-5 pt-5 pb-4">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-3xl font-black text-white tracking-tight">Debts</h1>
        ...
        {debts.length > 0 && (
          <div className="grid grid-cols-2 gap-3 mb-1">
            <div className="bg-slate-800/60 rounded-2xl p-4 border border-slate-700/50">
              <p className="text-xs text-slate-500 uppercase tracking-widest mb-2">Total Debt</p>
              <p className="text-2xl font-black text-rose-400">{formatCurrency(totalBalance)}</p>
              <p className="text-xs text-slate-500 mt-1">{debts.length} account{debts.length !== 1 ? 's' : ''}</p>
            </div>
            <div className="bg-slate-800/60 rounded-2xl p-4 border border-slate-700/50">
              <p className="text-xs text-slate-500 uppercase tracking-widest mb-2">Monthly Mins</p>
              <p className="text-2xl font-black text-amber-400">{formatCurrency(totalMin)}</p>
              <p className="text-xs text-slate-500 mt-1">{paidDebts.length} of {debts.length} paid</p>
            </div>
          </div>
        )}
      </div>
      <div className="px-5 space-y-3">
        {sorted.length === 0 ? (
          <div className="text-center py-16 text-slate-500">
            ...
            <p className="text-base">No debts added. Tap Add to track loans, credit cards, etc.</p>
          </div>
        ) : sorted.map((debt) => (
          <DebtCard key={debt.id} debt={debt} month={mk} onTogglePaid={toggleDebtPaid} onEdit={setEditItem} onDelete={deleteDebt} />
        ))}
      </div>
      {showAdd && (
        <Modal title="Add Debt" onClose={() => setShowAdd(false)}>
          <DebtForm onSave={(data) => { addDebt(data); setShowAdd(false); }} onCancel={() => setShowAdd(false)} />
        </Modal>
      )}
      {editItem && (
        <Modal title="Edit Debt" onClose={() => setEditItem(null)}>
          <DebtForm initial={editItem} onSave={(data) => { updateDebt(editItem.id, data); setEditItem(null); }} onCancel={() => setEditItem(null)} />
        </Modal>
      )}
    </div>
  );
