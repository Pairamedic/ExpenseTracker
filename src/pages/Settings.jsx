      ...
      <div className="px-5 pt-8 pb-6">
        ...
        <section className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">My Profile</h2>
          </div>
          <div>
        ...
        <section className="mb-6 bg-slate-800/60 rounded-2xl border border-slate-700/50 p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">Spouse / Partner</h2>
            </div>
            <div
              className={`w-11 h-6 rounded-full transition-colors relative cursor-pointer ${form.spouseEnabled ? 'bg-indigo-500' : 'bg-slate-700'}`}
              onClick={() => set('spouseEnabled', !form.spouseEnabled)}
            >
        ...
        <button
          onClick={save}
          className={`w-full py-3.5 rounded-xl font-semibold transition-all mb-6 ${saved ? 'bg-emerald-600 text-white' : 'bg-indigo-600 hover:bg-indigo-500 text-white'}`}
        >
        ...
        <section className="bg-rose-950/30 border border-rose-900/50 rounded-2xl p-4">
          <h2 className="text-sm font-semibold text-rose-400 uppercase tracking-wide mb-3 flex items-center gap-2">
          ...
          <button onClick={() => setShowClearConfirm(true)}
            className="flex items-center gap-2 text-sm text-rose-400 border border-rose-800/50 px-4 py-2 rounded-xl hover:bg-rose-900/30 transition-colors">
      ...
      {showClearConfirm && (
        <Modal title="Clear All Data?" onClose={() => setShowClearConfirm(false)}>
          ...
          <div className="flex gap-3">
            <button onClick={() => setShowClearConfirm(false)} className="flex-1 py-3 rounded-xl border border-slate-700 text-slate-300 hover:bg-slate-800 transition-colors">
              Cancel
            </button>
            <button onClick={clearAll} className="flex-1 py-3 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-semibold transition-colors">
              Delete Everything
            </button>
          </div>
        </Modal>
