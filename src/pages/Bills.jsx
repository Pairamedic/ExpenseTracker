  ...
  return (
    <div className={`relative bg-slate-800/60 rounded-2xl border transition-all ${isPaid ? 'border-emerald-800/50 opacity-75' : 'border-slate-700/50'}`}>
      <div className="flex items-start gap-3 p-4">
        <button onClick={() => onToggle(bill.id, mk)}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <p className={`text-lg font-bold flex-shrink-0 ${isPaid ? 'text-emerald-400' : 'text-white'}`}>{formatCurrency(bill.amount)}</p>
          </div>
          <div className="flex items-center justify-between mt-2 gap-2">
            <div className="flex items-center gap-2">
              {dueDateLabel && <span className={`text-xs font-medium ${dueDateColor}`}>{dueDateLabel}</span>}
      ...
      {menuOpen && (
        <>
          ...
          <div className="absolute right-3 top-12 z-50 bg-slate-800 border border-slate-700 rounded-xl shadow-xl overflow-hidden min-w-[140px]">
            <button onClick={() => { onEdit(bill); setMenuOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-3 text-sm text-slate-300 hover:bg-slate-700 transition-colors"><Pencil size={14} /> Edit</button>
            <button onClick={() => { onDelete(bill.id); setMenuOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-3 text-sm text-rose-400 hover:bg-slate-700 transition-colors"><Trash2 size={14} /> Delete</button>
          </div>
        </>
      )}
    </div>
        ...
        <div className="flex gap-2">
          {['all', 'unpaid', 'paid'].map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors capitalize ${filter === f ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}>{f}</button>
          ))}
        </div>
      </div>
      <div className="px-5 space-y-3">
        {sorted.length === 0 ? (
          <div className="text-center py-16 text-slate-500"><Receipt size={40} className="mx-auto mb-3 opacity-30" /><p className="text-sm">No bills yet. Tap Add to get started.</p></div>
        ) : sorted.map((bill) => (
          <BillCard key={bill.id} bill={bill} mk={mk} onToggle={toggleBillPaid} onEdit={setEditBill} onDelete={deleteBill} />
        ))}
      </div>
      {showAdd && (
        <Modal title="Add Bill" onClose={() => setShowAdd(false)}>
          <BillForm onSave={(data) => { addBill({ ...data, month: mk }); setShowAdd(false); }} onCancel={() => setShowAdd(false)} />
        </Modal>
      )}
      {editBill && (
        <Modal title="Edit Bill" onClose={() => setEditBill(null)}>
          <BillForm initial={editBill} onSave={(data) => { updateBill(editBill.id, data); setEditBill(null); }} onCancel={() => setEditBill(null)} />
        </Modal>
      )}
    </div>
  );
