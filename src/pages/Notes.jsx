...
function NoteForm({ initial = {}, onSave, onCancel }) {
  ...
  const handleSubmit = (e) => { e.preventDefault(); if (!content.trim()) return; onSave({ title: title.trim(), content: content.trim() }); };
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div>
        ...
        <input className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
          placeholder="e.g. Savings goal, Tax notes..." value={title} onChange={(e) => setTitle(e.target.value)} />
      </div>
      <div>
      ...
      {menuOpen && (
        <>
          ...
          <div className="absolute right-3 top-10 z-50 bg-slate-800 border border-slate-700 rounded-xl shadow-xl overflow-hidden min-w-[150px]">
            <button onClick={() => { onPin(note.id); setMenuOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-3 text-sm text-slate-300 hover:bg-slate-700 transition-colors">
              {note.pinned ? <PinOff size={14} /> : <Pin size={14} />}{note.pinned ? 'Unpin' : 'Pin to top'}
            </button>
            <button onClick={() => { onEdit(note); setMenuOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-3 text-sm text-slate-300 hover:bg-slate-700 transition-colors"><Pencil size={14} /> Edit</button>
            <button onClick={() => { onDelete(note.id); setMenuOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-3 text-sm text-rose-400 hover:bg-slate-700 transition-colors"><Trash2 size={14} /> Delete</button>
          </div>
        </>
      )}
    </div>
    ...
    <div className="pb-32">
      ...
      <div className="px-5 space-y-3">
        {filtered.length === 0 && notes.length === 0 ? (
          ...
          <NoteCard key={note.id} note={note} onEdit={setEditNote} onDelete={deleteNote} onPin={toggleNotePin} />
        ))}
      </div>
      {showAdd && <Modal title="New Note" onClose={() => setShowAdd(false)}><NoteForm onSave={(data) => { addNote(data); setShowAdd(false); }} onCancel={() => setShowAdd(false)} /></Modal>}
      {editNote && <Modal title="Edit Note" onClose={() => setEditNote(null)}><NoteForm initial={editNote} onSave={(data) => { updateNote(editNote.id, data); setEditNote(null); }} onCancel={() => setEditNote(null)} /></Modal>}
    </div>
  );
}
