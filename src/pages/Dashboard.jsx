...
function getMonthlyIncome(incomeItems) {
  return incomeItems.reduce((sum, item) => {
    const multiplier = item.frequency === 'weekly' ? 4.33 : item.frequency === 'biweekly' ? 2.17 : 1;
      ...
      <div className="px-5 grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="bg-slate-800/60 rounded-2xl p-4 border border-slate-700/50">
          <div className="flex items-center gap-2 text-rose-400 mb-2">
            <Receipt size={16} />
            <span className="text-xs font-medium uppercase tracking-wide">Bills</span>
          </div>
        ...
        <div className={`col-span-2 rounded-2xl p-4 border ${remaining >= 0 ? 'bg-indigo-900/30 border-indigo-700/50' : 'bg-rose-900/30 border-rose-700/50'}`}>
          <div className={`flex items-center gap-2 mb-2 ${remaining >= 0 ? 'text-indigo-400' : 'text-rose-400'}`}>
            <Wallet size={16} />
            <span className="text-xs font-medium uppercase tracking-wide">Left after bills</span>
          </div>
      ...
      <div className="px-5 space-y-3">
        <div className="bg-emerald-900/20 border border-emerald-800/40 rounded-2xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-emerald-300">Paid</p>
        ...
        <div className="bg-amber-900/20 border border-amber-800/40 rounded-2xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-amber-300">Unpaid</p>
