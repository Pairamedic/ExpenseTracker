...
export default function IncomeForm({ initial = {}, onSave, onCancel, spouseEnabled, spouseName }) {
  const [form, setForm] = useState({
    source: '', amount: '', isRecurring: true, frequency: 'biweekly', person: 'me', notes: '',
    ...initial,
  });
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div>
        ...
        <input className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
          placeholder="e.g. Paycheck, Freelance, Side job" value={form.source} onChange={(e) => set('source', e.target.value)} required />
      </div>
      <div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => set('person', 'me')}
              className={`flex-1 py-2.5 rounded-xl border text-sm font-medium transition-colors ${form.person === 'me' ? 'bg-indigo-600 border-indigo-500 text-white' : 'border-slate-700 text-slate-400 hover:bg-slate-800'}`}>Me</button>
      ...
      <div>
        ...
        <input className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
          placeholder="Optional" value={form.notes} onChange={(e) => set('notes', e.target.value)} />
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="submit" className="flex-1 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition-colors">Save</button>
      </div>
    </form>
  );
}
